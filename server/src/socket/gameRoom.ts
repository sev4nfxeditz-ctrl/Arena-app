...
export function setupGameRoom(
  ...
  socket.on('make_move', async (data) => {
    ...
    if (!room) {
      ...
      const roomData = await redis.get(keys.gameRoom(roomId));
      if (!roomData) {
    ...
    room.moveHistory.push({ ...move, notation: result.notation, timestamp: Date.now() });
    ...
    await redis.set(keys.gameState(roomId), JSON.stringify(room.gameState), 'EX', 7200);
  });
  ...
  socket.on('accept_draw', async (data) => {
    ...
    await endGame(io, room, null, 'stalemate'); // null winner = draw
  });
  ...
  // ---- AI MOVE (triggered after player moves in AI game) ----
    ...
    setTimeout(() => {
      switch (room.gameType) {
        ...
        case 'checkers':
          ...
          break;
        case 'chess':
          // Chess AI is handled client-side via Stockfish WASM
          // Server fallback: pick a random legal move
  ...
  socket.on('spectate', (data) => {
    ...
    const room = activeRooms.get(data.roomId);
    if (room) {
      io.to(data.roomId).emit('spectator_joined', { count: 0 }); // TODO: track count
    }
  });
...
async function endGame(
  ...
  // Update ELO for ranked matches (not AI games)
  let eloChanges;
  if (room.mode === 'ranked' && room.player2.userId !== 'AI_BOT') {
    try {
      const eloResult = await updatePlayerRatings(
        room.player1.userId, room.player2.userId,
        room.gameType, winnerId
      );
  ...
  await redis.del(keys.gameState(room.roomId));
  await redis.del(keys.gameRoom(room.roomId));

  console.log(`🏁 Game over: ${room.roomId} (${reason})`);
}
